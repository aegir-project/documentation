...
**Note: Do not use this release if you depend on HTTPS support via [Aegir HTTPS](https://www.drupal.org/project/hosting_https).  While we fixed [the bug that's causing problems](https://www.drupal.org/node/3020747), we haven't cut a new release yet. Please either use an older release, the development branch, or wait for the new release, which will be out shortly.  Thanks, and sorry for the trouble.**
